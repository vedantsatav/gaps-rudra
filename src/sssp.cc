...
pvector<WeightT> DeltaStep(const WGraph &g, NodeID source, WeightT delta, pvector<NodeID> &new_ids, bool isReordered = false) {
  Timer t;
...
t.Start();
  ...
  // g = WeightedBuilder::generateNeighbourMapping(g1, false, new_ids,true,true); //second arg = true to useOutDeg
    g=WeightedBuilder::generateNumberNodeNeighbourMapping(g1, false, new_ids,true);
    t.Stop();
...
else if (flag == 2)
{
  ...
  t.Start();
 g = WeightedBuilder::generateDBGMapping(g1, false, new_ids,true); //second arg = true to useOutDeg
    t.Stop();
  ...
  g = WeightedBuilder::generateNumberNodeNeighbourMapping2(g1, false, new_ids,true); //second arg = true to useOutDeg
    t.Stop();
//for all sideways numbering
  ...
  {
    t.Start();
    g = WeightedBuilder::generateDBGMappingUndirected(g1, false, new_ids, true);
    t.Stop();
  }
else if(flag == 5)
  {
    ...
    t.Start();
    g = WeightedBuilder::generateDBGMappingUndirected(g1, true, new_ids,true); //second arg = true to useOutDeg
    t.Stop();
  }
  else if(flag == 6)
  {
    // Graph g1 = b.MakeGraph();
    t.Start();
    g = WeightedBuilder::undirectedgenerateNumberNodeNeighbourMapping(g1, true, new_ids,true); //second arg = true to useOutDeg
    t.Stop();
  }
...
//  
 ...
 // WGraph g1 = b.MakeGraph();
  // WGraph g = WeightedBuilder::generateDBGMapping(g1, true); //second arg = true to useOutDeg



  // SourcePicker<WGraph> sp(g1, 0);
  SourcePicker<WGraph> sp(g1, cli.start_vertex());

  auto SSSPBound = [&sp, &cli,  &new_ids, &isReordered] (const WGraph &g) {
    return DeltaStep(g, sp.PickNext(), cli.delta(),  new_ids, isReordered);
  };
  SourcePicker<WGraph> vsp(g, cli.start_vertex());
  // SourcePicker<WGraph> vsp(g, 0);
  auto VerifierBound = [&vsp] (const WGraph &g, const pvector<WeightT> &dist) {
    return SSSPVerifier(g, vsp.PickNext(), dist);
  };
  BenchmarkKernel(cli, g, SSSPBound, PrintSSSPStats, VerifierBound, t.Seconds());
  ...
  //  {
  //     for(WNode wn: g.out_neigh(u)){
  //       cout << "u: " << u << " wn: " << wn << " weight: " << wn.w << endl;
  //     }
  //  }
  return 0;
}
