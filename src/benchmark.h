...
void BenchmarkKernel(const CLApp &cli, const GraphT_ &g,
                     ...
                     VerifierFunc verify, double reorder_time = 0) {
  g.PrintStats();
  double total_seconds = 0;
  Timer trial_timer;
#ifdef VTUNE
  __itt_resume(); //start recording the performance data
#endif

  cout << "Reordering time from benchmark kernel: " << reorder_time << endl;
...
#ifdef VTUNE
  __itt_pause(); //stop recording the performance data
#endif
PrintTime("Reordering time: ", reorder_time); 
PrintTime("Without reorder: Average Time", (total_seconds) / cli.num_trials());
  PrintTime("With reorder: Average Time", (total_seconds + reorder_time) / cli.num_trials());
}
