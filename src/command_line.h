...
class CLBase {
 protected:
  ...
  char** argv_;
  std::string name_;
  std::string get_args_ = "f:g:hk:su:m:p:q";
 ...
 public:
  CLBase(int argc, char** argv, std::string name = "") :
    ...
    AddHelpLine('q', "radius", "For BFS");
 
  }

  bool ParseArgs() {
    std::cout<<"CLBase::ParseArgs entered"<<std::endl;
    signed char c_opt;
    extern char *optarg;          // from and for getopt
    while ((c_opt = getopt(argc_, argv_, get_args_.c_str())) != -1) {
      //std::cout<<"optarg= "<< atoi(optarg)<<std::endl;
      std::cout << c_opt << "\t" <<  optarg << "\n";
      //std::cout<<"returned getopt()"<<std::endl;
  ...
  void virtual HandleArg(signed char opt, char* opt_arg) {
    std::cout<<"CLBase::HandleArg entered"<<std::endl;
    switch (opt) {
      ...
      case 'q': radius = atoi(opt_arg); std::cout<<"radius set"<<std::endl;                    break;
